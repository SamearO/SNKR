...
export const MyAreaChart = (props) => {
  ...
  class Data{
    ...
    constructor(data, size, startDate, endDate){
      this.data = data
      this.size = size
    ...
    formatdate(){
      ...
      for (var i = 0; i < this.data.length; i++) {
        this.data[i].ProductActivity__createdAt = moment(
        this.data[i].ProductActivity__createdAt
        ).format("YYYY-MM-DD");
    }
    }
    ...
    filterByDateAndSize(size, startDate, endDate){
      ...
      for (var i = 0; i < this.data.length; i++) {
        var comparisondate = new Date(this.data[i].ProductActivity__createdAt);
        if (
          this.data[i]["ProductActivity__shoeSize"] == size &&
  ...
  let a = new Data(data, props.size, props.startDate, props.endDate)

  a.sortDates()
  
  // sets data to data filtered by the filtered component
  a.data = a.filterByDateAndSize(props.size, props.startDate, props.endDate);
