...
class predict():
    ...
    def predictorRecord(self):
        arr = []
        for i in self.grabsales():
    ...
    def findsz(self, arr, sz):
        for i in range(len(arr) -1):
            if arr[i].size == sz:
                return arr[i].uuid


    def grabProductInfo(self, product, size):
        headers = {
        "user-agent":
        ...
        sizeid = self.findsz(variantArray, size)
        return sizeid

    def newjsonpredict(self):
        filtered = self.filterarr(self.size, self.predictorRecord())
    ...
    def jsonpredict(self):
        json = self.grabseries()
